...
internal actual object SimklAuthStorage {
    ...
    actual fun saveMetadataPayload(payload: String) {
        preferences?.edit()?.putString(ProfileScopedKey.of(METADATA_KEY), payload)?.commit()
    }
    ...
    private fun loadEncrypted(key: String): String? {
        ...
        return runCatching { decrypt(stored) }
            .onFailure { preferences?.edit()?.remove(scopedKey)?.commit() }
            .getOrNull()
    }
    ...
    private fun saveEncrypted(key: String, value: String?) {
        ...
        if (value.isNullOrBlank()) {
            editor.remove(scopedKey).commit()
        } else {
            editor.putString(scopedKey, encrypt(value)).commit()
        }
    }
